...
Build this step by step. Start simple, then handle edge cases.`,
  ...
  _apply() {
    ...
    else if (this.operator === "-") result = this.stored - current;
    else if (this.operator === "*") result = this.stored * current;
    else if (this.operator === "/") {
      result = current === 0 ? Infinity : this.stored / current;
    }
...
- Multiple keys are independent`,
  ...
  isAllowed(key) {
    ...
    const timestamps = (this.requests.get(key) || [])
      .filter(t => t > windowStart); // remove expired

    if (timestamps.length < this.limit) {
      // Allow: add current timestamp and update
      timestamps.push(now);
